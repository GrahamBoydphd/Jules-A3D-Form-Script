  ...
  const mockEvent = {
    response: {
      ...
      getItemResponses: () => [
        ...
        { getItem: () => ({ getTitle: () => "How are staff included in the financial benefits of shareholding, so that they have a share of dividends and any growth in the company's value. (Tick all that apply)" }), getResponse: () => "Only if staff buy or sell shares; they're included because they have invested, not because they are staff; there's no difference" },
        { getItem: () => ({ getTitle: () => "How are staff included in the governance aspect of shareholding - the staff share of governance / voting rights" }), getResponse: () => "Staff only get voting rights when they buy voting shares just as any investor does" },
        ...
        { getItem: () => ({ getTitle: () => "How well does your company make explicit and express in practice that its legal personhood means that it is legally a free person, not property, not an ownable thing, and especially that it is not owned by the shareholders?" }), getResponse: () => "1" },
        ...
        { getItem: () => ({ getTitle: () => "How is employee performance most often discussed or evaluated?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "What happens when someone struggles to meet expectations?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "What happens when someone exceeds expectations consistently?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "How frequently do people receive feedback focused on growth?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "What is the default response to employee mistakes?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "What role does psychological safety play in team culture?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "How visible is inner development (current stage and developmental edge) in meetings or strategy reviews?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "How is success defined for employees?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "How are development resources allocated?" }), getResponse: () => "1" },
        ...
        { getItem: () => ({ getTitle: () => "How well are individual purposes and company purpose actively aligned in order to benefit both?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "How is inner development woven into the company’s identity and operations?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "How are people included in setting their personal development goals within the company?" }), getResponse: () => "1" },
        { getItem: () => ({ getTitle: () => "How are power and decision-making influenced by people’s relational and emotional intelligence?" }), getResponse: () => "1" },
        ...
        { getItem: () => ({ getTitle: () => "How is conflict between peers or upward (from junior to senior roles) seen and treated in the organisation?" }), getResponse: () => "1" },
